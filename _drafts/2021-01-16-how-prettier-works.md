---
layout: post
title: How Prettier Works
---

I've been working on [Prettier for Ruby](https://github.com/prettier/plugin-ruby) for about 2.5 years now, and the way that it functions has changed quite a bit over time. In this post I'm going to go through the basic idea of how it works today, as well as provide some background as to how and why it got into the state it's currently in. Here's the basic flow of how [prettier](https://prettier.io/) works under the hood:

* The CLI is invoked via command-line or editor plugin
* We determines which files are going to be processed
* For each file, a parser is found that matches either the file extension (e.g., `.rb`), the interpreter in the shebang declaration (e.g., `#!/usr/bin/env ruby`), or the filename itself (e.g., `Gemfile`)
* The parser can
